...
var QCMeditors = {
    ...
    setEditable:function(yn){
        ...
        if(yn)document.getElementById("questions").classList.add("content-editable");
        else document.getElementById("questions").classList.remove("content-editable");
    ...
    myMove: function (direction) {
        ...
        // on cache les datas
        datas =  {};
        // on cache la liste des réponses (que l'on affiche ensuite)
        document.querySelector("#editor"+QCMeditors.qFocusOn+" ol").classList.add('invisible');
        ...
        function frame() {
            if (pos >= espaceToGo) {
                ...
                document.querySelector("#editor"+QCMeditors.qFocusOn+" ol").classList.remove('invisible');
            } else {
                pos = pos + pas;
    ...
    addStyle: function (questionNumber) {
        ...
        for (var i = 0; i < s.length; i++) {
            document.querySelector("#editor"+questionNumber+" ol > li:nth-of-type("+s[i]+")").classList.add("correct");
    ...
    removeStyle: function (questionNumber) {
        ...
        document.querySelectorAll("#editor"+questionNumber+" ol > li").forEach((el)=>{el.classList.remove("correct");})
    ...
    replaceQuestions: function (json) {
        ...
        QCMeditors.fileJustLoaded = true;
        //document.querySelector("#editor0 ol").classList.add("visible");
        //document.getElementById('animate').addEventListener('click', QCMeditors.loadCKEDITOR);
...
var users = {
    ...
    afficheReponses: function (doIt) {
        ...
        if (utils.class('colonnes') === "cache" && doIt) {
            ...
            btn.src = "css/img/qcmcam2/column-chart.svg";
     ...
     */
     scanner: function (doIt) {
        ...
        if (start) { // démarrage du scan
            ...
            document.getElementById("btna").src = "css/img/qcmcam2/scan.svg";
    ...
    tick: function () {
        ...
        if (camera.video.readyState === 4) {
            if (camera.videoReady === false && camera.video.videoWidth > 0) {
                ...
                // camera.canvas.style.width = "46vw";
                camera.canvas.style.height = "25vh";
                camera.canvas.style.width = Math.round(25 * parseInt(camera.video.videoWidth) / parseInt(camera.video.videoHeight))+"vh"
                // mise à l'échelle de la hauteur en fonction du format de la vidéo
                //var hauteurImage = Math.round(46 * parseInt(camera.video.videoHeight) / parseInt(camera.video.videoWidth));
                // camera.canvas.style.height = hauteurImage + "vw";
