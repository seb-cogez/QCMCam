$(function(){
	...
	var filemanager = $('.filemanager'),
		...
		ouvrir = function(url) {
			if (window.opener) {
				window.opener.QCMeditors.loadFile(encodeURI('repository/'+url),true);
				//self.close();
			} else {
				location.href = 'https://qcmcam.net/?url='+encodeURI('repository/'+url);
			}
		}
		...
		function searchData(data, searchTerms) {
			...
			data.forEach(function(d){
				if(d.type === 'folder') {
					...
					if(d.name.toLowerCase().match(searchTerms)) {
						files.push(d);
					} else if(d.desc.toLowerCase().match(searchTerms)) {
						files.push(d);
					}
				}
			});
	...
	loadFile = function(){
		$("#invisible").load("arbo.json", function(){readData(JSON.parse($('#invisible').html()));});
	};
	// Start by fetching the file data from scan.php with an AJAX request
	$.ajax({url:'arbo.json', success:readData, error:loadFile});	
});
